...
> **▶️ 다음 작업**: Slice 3 · F3.4 (평가지표 + 임계값 보정) — `etl/analysis/metrics.py`에서 **scikit-learn**(`confusion_matrix`·`balanced_accuracy_score`·`precision_recall_fscore_support`)으로 F3.3의 감성 예측 vs `voted_up`을 정량 평가하고, `etl/analysis/split.py`(**scikit-learn** `train_test_split(stratify=...)`)로 계층 분할한다. 실행 진입점은 `scripts/calibrate.py`. F3.3에서 저장한 `matches_voted_up`·`sentiment_score`를 근거로 balanced accuracy·F1을 산출해 감성 모델을 정량 검증하는 단계.
